...
from meditrack.models import Obat, StockMovement, TransaksiPenjualan, DetailTransaksi
...
class InventoryTests(APIData):
    ...
    def test_expiry_boundaries_and_missing_date(self):
        ...
        for days, expected in [(-1, 'expired'), (0, 'near_expiry'), (30, 'near_expiry'), (31, 'safe')]:
    ...
    def test_restock_records_actor_before_after_and_note(self):
        response = self.client.post(f'/obat/{self.obat.pk}/restock/', {'quantity': 7, 'note': 'Delivery'})
        self.assertEqual(response.status_code, 302)
        self.obat.refresh_from_db()
        self.assertEqual(self.obat.stok, 17)
        movement = StockMovement.objects.get()
        self.assertEqual((movement.tipe, movement.quantity, movement.stock_before, movement.stock_after), ('IN', 7, 10, 17))
    ...
    def test_invalid_restock_quantity_has_no_side_effects(self):
        for quantity in [0, -1, 'abc', '1.5', 2147483648]:
            self.assertEqual(self.client.post(f'/obat/{self.obat.pk}/restock/', {'quantity': quantity}).status_code, 200)
    ...
    def test_restock_is_atomic_when_audit_write_fails(self):
        with patch('meditrack.services.StockMovement.objects.create', side_effect=RuntimeError):
    ...
    def test_checkout_records_sale_once_and_rolls_back_on_later_failure(self):
        self.client.post('/api/transaksi/cart/add/', {'obat': self.obat.pk, 'jumlah': 2}, format='json')
        second = Obat.objects.create(nama_obat='Second', kategori=self.kategori, supplier=self.supplier, harga=1, stok=1)
        self.client.post('/api/transaksi/cart/add/', {'obat': second.pk, 'jumlah': 1}, format='json')
        second.stok = 0
        second.save()
        ...
        second.stok = 1
        second.save()
        self.assertEqual(self.client.post('/api/transaksi/cart/checkout/').status_code, 200)
        ...
        self.assertEqual(movement.reference, f'transaksi:{TransaksiPenjualan.objects.get().pk}')
        self.assertEqual(self.client.post('/api/transaksi/cart/checkout/').status_code, 400)
        self.assertEqual(StockMovement.objects.count(), 2)

    def test_kasir_records_sale(self):
        response = self.client.post('/kasir/', {'form-TOTAL_FORMS': 1, 'form-INITIAL_FORMS': 0, 'form-0-obat': self.obat.pk, 'form-0-jumlah': 2})
        self.assertEqual(response.status_code, 302)
    ...
    def test_api_manual_adjustment_and_metadata_only_update(self):
        ...
        movement = StockMovement.objects.get()
        self.assertEqual((movement.tipe, movement.quantity, movement.stock_before, movement.stock_after), ('ADJUSTMENT', 6, 10, 4))
        self.client.patch(f'/api/obat/{self.obat.pk}/', {'nama_obat': 'New name'}, format='json')
        self.assertEqual(StockMovement.objects.count(), 1)

    def test_html_manual_adjustment(self):
        response = self.client.post(f'/obat/{self.obat.pk}/edit/', {'nama_obat': self.obat.nama_obat, 'kategori': self.kategori.pk, 'supplier': self.supplier.pk, 'harga': self.obat.harga, 'stok': 6, 'minimum_stock': 3})
        self.assertEqual(response.status_code, 302)
    ...
    def test_expired_product_rejected_at_add_and_checkout(self):
        ...
        self.obat.save()
        self.assertEqual(self.client.post('/api/transaksi/cart/add/', {'obat': self.obat.pk, 'jumlah': 1}, format='json').status_code, 400)
        self.assertEqual(self.client.post('/api/transaksi/cart/checkout/').status_code, 400)
        self.assertFalse(StockMovement.objects.exists())
    ...
    def test_database_constraints_zero_quantity_and_negative_price(self):
        ...
        with self.assertRaises(IntegrityError), transaction.atomic():
            DetailTransaksi.objects.create(transaksi=trx, obat=self.obat, jumlah=0, subtotal=0)
        with self.assertRaises(IntegrityError), transaction.atomic():
    ...
    def test_restock_staff_only_and_inventory_filter(self):
        ...
        self.assertEqual(self.client.post(f'/obat/{self.obat.pk}/restock/', {'quantity': 2}).status_code, 403)

    def test_history_protects_catalog_deletion(self):
        restock(self.obat.pk, 1, self.user)
        for kind, pk in [('obat', self.obat.pk), ('kategori', self.kategori.pk), ('supplier', self.supplier.pk)]:
