...
class DashboardTests(APIData):
    ...
    def test_paid_kpis_chart_and_top_products_exclude_other_users(self):
        today = timezone.now()
        cases = [
            (self.user, "PAID", 0, 100, 2),
            (self.user, "PENDING", 0, 300, 3),
            (self.user, "PAID", 6, 50, 1),
            (self.user, "PAID", 7, 25, 1),
            (self.other, "PAID", 0, 900, 9),
        ]
        for owner, state, days, total, quantity in cases:
            trx = TransaksiPenjualan.objects.create(
                user=owner, status=state, total_harga=total
            )
            TransaksiPenjualan.objects.filter(pk=trx.pk).update(
                tanggal=today - timedelta(days=days)
            )
            DetailTransaksi.objects.create(
                transaksi=trx, obat=self.obat, jumlah=quantity, subtotal=total
            )
        ...
        self.assertTrue(
            all(trx.user_id == self.user.pk for trx in context["recent_transactions"])
        )
