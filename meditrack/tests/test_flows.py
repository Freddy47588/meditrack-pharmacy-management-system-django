...
from meditrack.models import Obat, DetailTransaksi, TransaksiPenjualan
...
class FlowRegressionTests(APIData):
    ...
    def add(self, quantity=2):
        return self.client.post('/api/transaksi/cart/add/', {'obat': self.obat.pk, 'jumlah': quantity}, format='json')
    ...
    def test_detail_and_cart_ownership_all_methods(self):
        ...
        item = DetailTransaksi.objects.create(transaksi=trx, obat=self.obat, jumlah=1, subtotal=self.obat.harga)
        ...
        for prefix in ['/api/detail-transaksi/', '/api/transaksi/cart/items/']:
            for method in ['patch', 'delete']:
                self.assertEqual(getattr(self.client, method)(f'{prefix}{item.pk}/', {}, format='json').status_code, 404)
        self.assertEqual(self.client.get(f'/api/detail-transaksi/{item.pk}/').status_code, 404)
        self.assertEqual(self.client.post('/api/detail-transaksi/', {'transaksi': trx.pk, 'obat': self.obat.pk, 'jumlah': 1}, format='json').status_code, 400)

    def test_checkout_rolls_back_earlier_item(self):
        self.add()
        second = Obat.objects.create(nama_obat='Second', kategori=self.kategori, supplier=self.supplier, harga=1, stok=2)
        self.client.post('/api/transaksi/cart/add/', {'obat': second.pk, 'jumlah': 2}, format='json')
        second.stok = 0
        second.save()
    ...
    def test_paid_transaction_and_detail_cannot_be_edited_or_deleted(self):
        ...
        for path in [f'/api/transaksi/{pk}/', f'/api/detail-transaksi/{item}/']:
            self.assertEqual(self.client.patch(path, {'jumlah': 1}, format='json').status_code, 400)
    ...
    def test_status_cannot_bypass_checkout(self):
        data = self.add().data
        response = self.client.patch(f"/api/transaksi/{data['id']}/", {'status': 'PAID'}, format='json')
        self.assertEqual(response.data['status'], 'DRAFT')

    def test_nonstaff_cannot_manage_catalog(self):
        self.assertEqual(self.client.patch(f'/api/obat/{self.obat.pk}/', {'stok': 100}, format='json').status_code, 403)

    def test_detail_total_and_transfer_protection(self):
        data = self.add().data
        trx = TransaksiPenjualan.objects.create(user=self.other)
        pk = data['detail'][0]['id']
        self.assertEqual(self.client.patch(f'/api/detail-transaksi/{pk}/', {'transaksi': trx.pk}, format='json').status_code, 400)
        self.client.patch(f'/api/detail-transaksi/{pk}/', {'jumlah': 4}, format='json')
        self.assertEqual(TransaksiPenjualan.objects.get(pk=data['id']).total_harga, Decimal('50002.00'))
...
class KasirTests(APIData):
    ...
    def payload(self, quantity=2):
        return {'form-TOTAL_FORMS': '1', 'form-INITIAL_FORMS': '0', 'form-0-obat': self.obat.pk, 'form-0-jumlah': quantity}
    ...
    def test_kasir_insufficient_stock_and_empty_submission(self):
        for quantity in [11, 0, -1, '']:
            self.assertEqual(self.client.post('/kasir/', self.payload(quantity)).status_code, 200)
            self.assertFalse(TransaksiPenjualan.objects.exists())
        self.obat.refresh_from_db()
        self.assertEqual(self.obat.stok, 10)

    def test_kasir_rolls_back_duplicate_over_stock(self):
        data = {**self.payload(6), 'form-TOTAL_FORMS': '2', 'form-1-obat': self.obat.pk, 'form-1-jumlah': 6}
    ...
    def test_html_ownership_and_draft_delete_does_not_restock(self):
        ...
        self.assertEqual(self.client.post(f'/transaksi/{other.pk}/hapus/').status_code, 404)
        trx = TransaksiPenjualan.objects.create(user=self.user)
        DetailTransaksi.objects.create(transaksi=trx, obat=self.obat, jumlah=2, subtotal=1)
