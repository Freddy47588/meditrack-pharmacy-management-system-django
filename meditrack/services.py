...
from .models import Obat, TransaksiPenjualan, DetailTransaksi, StockMovement
...
def checkout(pk, user):
    ...
    for item in items:
        ...
        StockMovement.objects.create(obat=obat, tipe='SALE', quantity=item.jumlah,
            stock_before=obat.stok, stock_after=obat.stok-item.jumlah, user=user,
...
def record_adjustment(obat, before, user, note='Perubahan stok manual'):
    if before != obat.stok:
        StockMovement.objects.create(obat=obat, tipe='ADJUSTMENT', quantity=abs(obat.stok-before),
            stock_before=before, stock_after=obat.stok, user=user, note=note)


@transaction.atomic
def restock(pk, quantity, user, note=''):
    if isinstance(quantity, bool) or not isinstance(quantity, int) or quantity <= 0:
        raise ValidationError({'quantity': 'Jumlah restock harus bilangan bulat positif.'})
    ...
    StockMovement.objects.create(obat=obat, tipe='IN', quantity=quantity, stock_before=before,
        stock_after=obat.stok, user=user, note=note)
    return obat
